<!DOCTYPE html>
<html lang="ru">

<head>

  <!-- Google Tag Manager -->
  <script>
    (function(w, d, s, l, i) {
      w[l] = w[l] || [];
      w[l].push({
        'gtm.start': new Date().getTime(),
        event: 'gtm.js'
      });
      var f = d.getElementsByTagName(s)[0],
        j = d.createElement(s),
        dl = l != 'dataLayer' ? '&l=' + l : '';
      j.async = true;
      j.src =
        'https://www.googletagmanager.com/gtm.js?id=' + i + dl;
      f.parentNode.insertBefore(j, f);
    })(window, document, 'script', 'dataLayer', 'GTM-TC86W4J');
  </script>
  <!-- End Google Tag Manager -->

  <meta charset="UTF-8">
  <meta name="viewport" content="width=1180">
  <meta http-equiv="X-UA-Compatible" content="ie=edge">
  <title>Личный кабинет | Бизнес-марафон ТРАНСФОРМАЦИЯ</title>

  <meta property="og:title" content="Бизнес-марафон ТРАНСФОРМАЦИЯ">
  <meta property="og:description" content="Друзья, иду на бесплатный марафон #трансформациябизнеса, кто со мной?">
  <meta property="og:url" content="http://xn--80aayoegldhg0a2a2j.xn--p1ai/">
  <meta property="og:image" content="http://xn--80aayoegldhg0a2a2j.xn--p1ai/img/share.jpg?2017-09-06">
  <link rel="image_src" href="http://xn--80aayoegldhg0a2a2j.xn--p1ai/img/share.jpg?2017-09-06">

  <link href="img/favicon.ico" type="image/x-icon" rel="shortcut icon">
  <link href="img/favicon.ico" type="image/x-icon" rel="icon">

  <script src="https://use.fontawesome.com/6dd73a0138.js"></script>
  <link rel="stylesheet" href="/css/datepicker.min.css">
  <link href="https://cdnjs.cloudflare.com/ajax/libs/fancybox/2.1.5/jquery.fancybox.min.css" rel="stylesheet">
  <link rel="stylesheet" href="/css/fonts.css">
  <link rel="stylesheet" href="/css/bootstrap-grid.css">
  <link rel="stylesheet" href="/css/main.css">

  <!--[if IE]><script src="http://html5shiv.googlecode.com/svn/trunk/html5.js"></script><![endif]-->
  <!--[if lte IE 8]><script src="http://phpbbex.com/oldies/oldies.js" charset="utf-8"></script><![endif]-->
  <!--[if lt IE 9]><script src="http://ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js"></script><![endif]-->
  <!--[if gte IE 9]><!-->
  <script src="http://ajax.googleapis.com/ajax/libs/jquery/2.1.4/jquery.min.js"></script>
  <!--<![endif]-->

</head>

<body>

  <!-- Google Tag Manager (noscript) -->
  <noscript><iframe src="https://www.googletagmanager.com/ns.html?id=GTM-TC86W4J" height="0" width="0" style="display:none;visibility:hidden"></iframe></noscript>
  <!-- End Google Tag Manager (noscript) -->

  <header>
    <div class="container">
      <div class="row header">
        <div class="col-lg-2">
          <img src="img/header-logo.png" alt="" class="img-fluid">
        </div>
        <ul class="col-lg-3 menu">
          <li><a href="#" class="active">Обучение</a></li>
          <li><a href="#">Рейтинг</a></li>
          <li><a href="#">Платежи</a></li>
          <li><a href="#">Партнерам</a></li>
          <li><a href="#">Правила</a></li>
        </ul>
        <div class="col-lg-3 support">
          <p>центр<br>поддержки<br>марафона</p>
          <div>
            <a href="tel:88001008926">8 800 100-89-26</a>
            <a href="mailto:transformation@synergy.ru">transformation@synergy.ru</a>
          </div>
        </div>
        <div class="col-lg-2 user">
          <div class="user__name"><a href="#">Илья Обломов</a></div>
          <div class="user__avatar"><img src="img/avatar.png" alt="" class="img-fluid"></div>
          <div class="user__close"><a href="#">Выйти</a></div>
        </div>
      </div>
    </div>
  </header>
  <section id="main" class="profile-main">
    <div class="container">
      <div class="row">
        <h1>Партнерам</h1>
        <div class="theme__bottom">
          <a href="#" class="active theme__button">
            <div><img src="img/theme-open.png" alt="" class="img-fluid"></div>
            <p>Назад</p>
          </a>
        </div>
      </div>
    </div>
  </section>
  <section id="statistics">
    <div class="statistics">
      <div class="container">
        <div class="row">
          <h2><a href="statistics.php">Статистика партнера</a></h2>
          <h2><a href="offers.php">Офферы <span>(12)</span></a></h2>
        </div>
      </div>
    </div>
  <div class="container">
    <div class="row funds">
      <div class="calendar col-12">
        <div class="calendar__name">Илья Обломов</div>
        <div class="calendar__period">
          <span>Результаты за период</span>
          <input type='text'
          class="datepicker-here"
          placeholder="Выберите период"
          data-range="true"
          data-show-other-months="false"
          data-select-other-months="false"
          data-multiple-dates="2"
          data-multiple-dates-separator=" - "
          >
        </div>
        <div class="calendar__choose">
          <img src="img/calendar.png" alt="">
          <span>Выбрать<br> период</span>
        </div>
      </div>
      <div class="fund col-12">
        <div class="fund__item">
          <span>Входящий<br>трафик</span>
          <div class="fund__value">150</div>
        </div>
        <div class="fund__item">
          <span>Количество<br>оплат</span>
          <div class="fund__value">20</div>
        </div>
        <div class="fund__item">
          <span>Ваша<br>выручка</span>
          <div class="fund__value">150000<span>руб.</span></div>
        </div>
        <div class="fund__item">
          <span>Средний<br>чек</span>
          <div class="fund__value">4000<span>руб.</span></div>
        </div>
        <div class="fund__item">
          <span>Сумма вашего<br>вознаграждения</span>
          <div class="fund__value">19500<span>руб.</span></div>
        </div>
        <a href="#" class="">Вывести средства</a>
      </div>
    </div>
    <div class="row summary">
      <h2>Сводная статистика по продуктам</h2>
      <table class="stat">
        <thead>
          <tr>
            <th>№&nbsp;<i class="fa fa-caret-down" aria-hidden="true"></i></th>
            <th>Название продукта&nbsp;<i class="fa fa-caret-down" aria-hidden="true"></i></th>
            <th>Кол-во оплат&nbsp;<i class="fa fa-caret-down" aria-hidden="true"></i></th>
            <th>Сумма оплат&nbsp;<i class="fa fa-caret-down" aria-hidden="true"></i></th>
            <th>% Комиссии&nbsp;<i class="fa fa-caret-down" aria-hidden="true"></i></th>
            <th>Сумма<br>Вознаграждения&nbsp;<i class="fa fa-caret-down" aria-hidden="true"></i></th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td>1.</td>
            <td>Трансформация Марафон (корпоративный пакет - 3)</td>
            <td>10</td>
            <td>300000 руб.</td>
            <td>50%</td>
            <td>127500 руб.</td>
          </tr>
          <tr>
            <td>1.</td>
            <td>Трансформация Марафон (корпоративный пакет - 3)</td>
            <td>10</td>
            <td>300000 руб.</td>
            <td>50%</td>
            <td>127500 руб.</td>
          </tr>
          <tr>
            <td>1.</td>
            <td>Трансформация Марафон (корпоративный пакет - 3)</td>
            <td>10</td>
            <td>300000 руб.</td>
            <td>50%</td>
            <td>127500 руб.</td>
          </tr>
          <tr>
            <td>1.</td>
            <td>Трансформация Марафон (корпоративный пакет - 3)</td>
            <td>10</td>
            <td>300000 руб.</td>
            <td>50%</td>
            <td>127500 руб.</td>
          </tr>
          <tr>
            <td>1.</td>
            <td>Трансформация Марафон (корпоративный пакет - 3)</td>
            <td>10</td>
            <td>300000 руб.</td>
            <td>50%</td>
            <td>127500 руб.</td>
          </tr>
        </tbody>
      </table>
      <a href="#" class="rating__show">
          Показать<span>+</span>еще
        </a>
    </div>
    <div class="row summary">
      <h2>Сводная статистика по продуктам</h2>
      <table class="stat">
        <thead>
          <tr>
            <th>№&nbsp;<i class="fa fa-caret-down" aria-hidden="true"></i></th>
            <th>Фио клиента&nbsp;<i class="fa fa-caret-down" aria-hidden="true"></i></th>
            <th>Номер счета&nbsp;<i class="fa fa-caret-down" aria-hidden="true"></i></th>
            <th>Выручка&nbsp;<i class="fa fa-caret-down" aria-hidden="true"></i></th>
            <th>Продукт&nbsp;<i class="fa fa-caret-down" aria-hidden="true"></i></th>
            <th>Дата оплаты&nbsp;<i class="fa fa-caret-down" aria-hidden="true"></i></th>
          </tr>
        </thead>
        <tbody>
          <tr>
            <td>1.</td>
            <td>Обломов Илья Ильич</td>
            <td>1974537823314</td>
            <td>300000 руб.</td>
            <td class="product">Трансформация<br><span>(базовый)</span></td>
            <td>01.11.2017</td>
          </tr>
          <tr>
            <td>1.</td>
            <td>Обломов Илья Ильич</td>
            <td>1974537823314</td>
            <td>300000 руб.</td>
            <td class="product">Трансформация<br><span>(базовый)</span></td>
            <td>01.11.2017</td>
          </tr>
          <tr>
            <td>1.</td>
            <td>Обломов Илья Ильич</td>
            <td>1974537823314</td>
            <td>300000 руб.</td>
            <td class="product">Трансформация<br><span>(базовый)</span></td>
            <td>01.11.2017</td>
          </tr>
          <tr>
            <td>1.</td>
            <td>Обломов Илья Ильич</td>
            <td>1974537823314</td>
            <td>300000 руб.</td>
            <td class="product">Трансформация<br><span>(базовый)</span></td>
            <td>01.11.2017</td>
          </tr>
          <tr>
            <td>1.</td>
            <td>Обломов Илья Ильич</td>
            <td>1974537823314</td>
            <td>300000 руб.</td>
            <td class="product">Трансформация<br><span>(базовый)</span></td>
            <td>01.11.2017</td>
          </tr>
        </tbody>
      </table>
      <a href="#" class="rating__show">
          Показать<span>+</span>еще
        </a>
    </div>
  </div>

  </section>
  <!-- <section id="sgf">
    <div class="container">
      <div class="row">
        <img src="img/sgf-logo.png" alt="" class="img-fluid">
      </div>
      <div class="row">
        <div class="sgf-title">
          Глобальное бизнес-событие года
        </div>
        <a href="http://synergyglobal.ru/" target="_blank" class="sgf-button">Узнать подробнее</a>
      </div>
    </div>
  </section> -->
  <footer>
    <div class="container">
      <div class="row footer">
        <div class="footer__logo">
          <p>Организатор</p>
          <img src="img/footer-logo.png" alt="" class="img-fluid">
        </div>
        <div class="footer__development">
          Разработка и продвижение:
          <a href="http://sydi.ru/" class="footer__development-link">Synergy Digital</a>
        </div>
        <div class="footer__contacts">
          <div class="footer__contact">
            <img src="img/phone.png" alt="" class="img-fluid">
            <a href="tel:88001008926">8 800 100-89-26</a>
          </div>
          <div class="footer__contact">
            <img src="img/mail.png" alt="" class="img-fluid">
            <a href="mailto:transformation@synergy.ru">transformation@synergy.ru</a>
          </div>
        </div>
      </div>
    </div>
  </footer>

  <!-- Payments popup start -->
  <div id="payments-popup" style="display: none;">
    <div class="popup-start">
      <img src="img/payments-popup.png" alt="">
      <div class="rate__title"></div>
      <div class="rate__price"></div>
    </div>
    <div class="popup-end">
      <ul></ul>
      <button class="payments-btn">Перейти на тариф</button>
    </div>
  </div>
  <!-- Payments popup end -->

  <script src="https://cdnjs.cloudflare.com/ajax/libs/fancybox/2.1.5/jquery.fancybox.pack.js"></script>
  <script src="/js/datepicker.min.js" charset="utf-8"></script>
  <script src="/js/main.js" charset="utf-8"></script>

</body>

</html>
